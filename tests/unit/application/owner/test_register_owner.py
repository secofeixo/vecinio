...
def test_registers_owner_and_persists_it() -> None:
    repository = InMemoryOwnerRepository()
    use_case = RegisterOwner(repository)

    owner_id = use_case.execute(
        nif="12345678Z",
        full_name="Jane Doe",
    ...
    persisted = repository.get_by_id(owner_id)
...
def test_registers_owner_without_phone() -> None:
    repository = InMemoryOwnerRepository()
    use_case = RegisterOwner(repository)
...
def test_execute_propagates_duplicate_nif_error_and_persists_nothing() -> None:
    repository = InMemoryOwnerRepository()
    use_case = RegisterOwner(repository)
    use_case.execute(
        nif="12345678Z",
...
def test_execute_propagates_value_error_for_invalid_nif_and_persists_nothing() -> None:
    repository = InMemoryOwnerRepository()
    use_case = RegisterOwner(repository)
...
def test_execute_propagates_value_error_for_invalid_email_and_persists_nothing() -> (
    None
):
    repository = InMemoryOwnerRepository()
    use_case = RegisterOwner(repository)
...
def test_execute_propagates_value_error_for_invalid_phone_and_persists_nothing() -> (
    None
):
    repository = InMemoryOwnerRepository()
    use_case = RegisterOwner(repository)
